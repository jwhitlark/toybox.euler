...
(defn max-left-right [d]
  (->> (partition 20 d)
       max-of-rows))

;; 48477312

(defn transpose-matrix [row-length d]
  ;; assumes square matrix
  (let [parts (partition row-length d)]
    (for [i (range row-length)]
      (map #(nth % i) parts))))
...
(def reduce-max (partial reduce max))

(defn take-diag [[r1 r2 r3 r4]]
  (for [i (range row-length)]
       [(get (vec r1) i 0)
        (get (vec r2) (+ 1 i) 0)
        (get (vec r3) (+ 2 i) 0)
...
;; how to do diagonals?
;; start at 0 0, go left
;; start at 0 0, go down
